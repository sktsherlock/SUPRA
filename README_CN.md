...
- **共享通道 (C)**：通过 concat(text, visual) → 消息传递学习通用图结构
- **独特文本通道 (Uₜ)**：将原始文本特征投影到嵌入空间（不经过图的消息传递）
- **独特视觉通道 (Uᵥ)**：将原始视觉特征投影到嵌入空间（不经过图的消息传递）
- **后期融合**：三个通道的 logits 平均：` (logits_C + logits_Ut + logits_Uv) / 3`
- **谱正交化 Hook**：对共享通道的梯度后处理，强制正交的更新方向
- **辅助损失（可选）**：每个分支的 cross-entropy 损失，增强独特通道的梯度信号

### 四路消融实验

| 变体 | 架构 | 正交化 | 辅助损失 |
|------|------|--------|----------|
| Ablate-None | C+Uₜ+Uᵥ 三通道 | ❌ (α=0) | ❌ |
| Ablate-Ortho | C+Uₜ+Uᵥ 三通道 | ✅ (α=1) | ❌ |
| Ablate-Aux | C+Uₜ+Uᵥ 三通道 | ❌ (α=0) | ✅ |
| SUPRA-Full | C+Uₜ+Uᵥ 三通道 | ✅ (α=1) | ✅ |
...
| 嵌入维度 | 共享嵌入维度 | 128, 256 |
| 共享深度 | 共享通道深度 | 1, 2, 3, 4 |
| Ortho Alpha | 谱正交化强度 | 0.0 (关闭), 1.0 (全开) |
| Aux Loss | 启用每分支辅助损失 | true/false |
...
├── run_ablation_study.sh     # 四路消融实验运行器
...
```bash
# 在所有数据集上运行四路消融（Movies, Grocery, Toys, Reddit-M）
./run_ablation_study.sh
...
| `--ortho_alpha` | 谱正交化强度（0=关闭, 1=全开） | 1.0 |
| `--use_aux_loss` | 启用辅助损失 | 关闭 |
| `--gpu` | GPU 设备 ID | 0 |
| `--n_runs` | 运行次数 | 3 |
