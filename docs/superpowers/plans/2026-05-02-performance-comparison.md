# Performance Comparison Plot — Implementation Plan
...
**Goal:** Create `GNN/Utils/plot_performance_comparison.py` — a faceted grouped bar chart showing MLP/MGCN/MGAT/MIGGT/NTSFormer accuracy across Movies/Grocery/RedditM, split by clip-roberta vs llama.

**Architecture:** Single standalone script with hardcoded data, argparse CLI, matplotlib PDF output. No external dependencies beyond standard library + matplotlib.

**Tech Stack:** Python 3, matplotlib, numpy, argparse, os

---

## File: `GNN/Utils/plot_performance_comparison.py`

**Files:**
- Create: `GNN/Utils/plot_performance_comparison.py`

No other files modified.

---

## Task 1: Write the full script

- [ ] **Step 1: Write `GNN/Utils/plot_performance_comparison.py`**
...
def plot_comparison(save_path: str = None) -> None:
    ...
    for ax_idx, dataset in enumerate(DATASETS):
        ...
        # group center at integer positions 0..n_models-1
        group_centers = np.arange(n_models)

        # clip-roberta bars: offset left by bar_width/2 + gap
        # llama bars: offset right by bar_width/2 + gap
...
```bash
cd D:/Project/SUPRA_2.0/SUPRA_2.0
python -m GNN.Utils.plot_performance_comparison --save_plot Results/perf_comparison_test.pdf
```

Expected: `[Saved] Plot → Results/perf_comparison_test.pdf` printed, no errors.

- [ ] **Step 3: Commit**

```bash
git add GNN/Utils/plot_performance_comparison.py
git commit -m "feat: add plot_performance_comparison for Intro section bar chart

- Faceted grouped bar chart: 3 datasets × 5 models × 2 encoders
- Independent Y-axis scales per dataset
- NTSFormer bars highlighted with bold outline
- Output as PDF vector format"
git push
```
